...
class SessionDAO(BaseDAO):
    ...
    @classmethod
    async def add(cls, token: str, user_id: int, created_at: datetime, expires_at: datetime) -> Session:
        async with async_session_maker() as session:
    ...
    async def delete_session(cls, token: str) -> Session:
        async with async_session_maker() as session:
            stmt = delete(cls.model).where(cls.model.token == token).returning(cls.model)
            result = await session.execute(stmt)
            await session.commit()
            return result.scalar()
    ...
    async def delete_all_sessions_for_user(cls, user_id: int) -> Session:
        async with async_session_maker() as session:
            stmt = delete(cls.model).where(cls.model.user_id == user_id).returning(cls.model)
            result = await session.execute(stmt)
            await session.commit()
            return result.scalar()